...
def open_image():
    ...
    canvas = ctk.CTkCanvas(image_window, width=img.width(), height=img.height())
    canvas.pack(expand=True, fill="both")

    # Create a label to display the image
    image_label = ctk.CTkLabel(canvas, image=img)
...
def run_sign_language_recognition_app():
    ...
    # Set up app bar
    app_bar = ctk.CTkFrame(root, height=50, bg_color="light blue")
    app_title_font = ctk.CTkFont(family="Arial", size=16)
    app_title = ctk.CTkLabel(app_bar, text="Indian Sign Language Recognition", fg_color="black", font=app_title_font)
    app_title.pack(side="top", anchor="center", padx=10, pady=10)
    app_bar.pack(fill="x", expand=False)

    # Set up camera frame
    camera_frame = ctk.CTkFrame(root, width=400, height=400)
    ...
    grayscale_label = ctk.CTkLabel(root)
    ...
    # Create a label for displaying the recognized sign and accuracy
    global output_label
    output_label = ctk.CTkLabel(root, text="", font=("Arial", 14))
    output_label.pack()

    # Create a label for displaying recognized text
    global text_label
    text_label = ctk.CTkLabel(root, text="Text: ", font=("Arial", 14))
    text_label.pack(anchor="w")

    # Button to clear the text
    clear_button = ctk.CTkButton(root, text="Clear", command=clear_text)
    clear_button.pack(anchor="w")

    # Button to open an image
    open_image_button = ctk.CTkButton(app_bar, text="Show Signs", command=open_image)
    ...
    def recognize_sign():
        ...
        if prediction_label != 'blank':
            ...
            output_label.configure(text=f"Sign: {prediction_label}\nAccuracy: {accu}%")
